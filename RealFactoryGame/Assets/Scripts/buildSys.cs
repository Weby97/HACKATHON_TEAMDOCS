...
{ 
    ...
    void Update()
    {
        ...
        {
            if (hit.transform.gameObject.tag == "anchor" && Input.GetMouseButtonDown(1) && buildScript.currentItem().holdingObj && buildScript.idaho && buildScript.currentItem().ammount > 0)//check if player is holding an obj
            {
